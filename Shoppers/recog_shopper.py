...
edge_path = os.path.join(dir_path, 'Edges')
piece_path = os.path.join(dir_path, 'Pieces')
...
def to_binary(file):
    ...
    img = cv2.imread(os.path.join(png_path,file))
    gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    bw_image = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 301, 5) 
#    cv2.imshow('image',bw_image)
#    cv2.waitKey(0)
    cv2.imwrite(os.path.join(binary_path,os.path.splitext(file)[0]+'.png'),bw_image)
...
#    cv2.waitKey(0)
    cv2.imwrite(os.path.join(edge_path,os.path.splitext(file)[0]+'.png'),edges)
...
def group_blobs(boundingboxes, sensitivity): #DBSCAN Clustering on Centroids ---------------
    ...
    clustering=DBSCAN(eps=sensitivity, min_samples=4).fit(centers)
...
def crop_clusters(img, clusters):
    ...
    for clust in clusters:
        ...
        cv2.imwrite(os.path.join(piece_path,str(counter)+'.png'),cropped)
        counter=counter+1
...
#BLOB GROUPING
    boundingboxes=blob_bounds(dir_path+'Binaries\\'+f)
    grouplist=group_blobs(boundingboxes, 290)
